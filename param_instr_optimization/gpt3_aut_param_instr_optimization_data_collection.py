...
csv_file = '220414_gpt3_aut_instr_param_optimization.csv' 
...
temperature = np.arange(.3, 1, .15)
presence_penalty = np.arange(-1.5, 2, 1.5)
frequency_penalty = np.arange(-1.5, 2, 1.5)
...
header = ['id', 'gpt3_id', 'gpt3_created', 'gpt3_model', 'timestamp', 'temperature', 'presence_penalty', 'frequency_penalty', 'instr_nr', 'instr_text', 'aut_object', 'gpt3_response']
writer.writerow(header)

# RAOUL: should I set a seed to make this reproducible?
# RAOUL: am I missing any important variables that we'll need for analysis here?
        ...
        for freq_pnlt in frequency_penalty:
            for aut_obj in aut_object:
                for instr_key, instr_val in instruction.items():
                    ...
                    # response = gpt3_response(eng, instr, max_tokens, temp, pres_pnlt, freq_pnlt, n)
                    rowid = rowid + 1
                    gpt3_response_text = f'response {rowid}'
                    # gpt3_response_text = response['choices'][0][text]
                    gpt3_id = 1 #response['id']
                    gpt3_created = 1 #response['created']
                    gpt3_model = 1 #response['model']
                    row = [rowid, gpt3_id, gpt3_created, gpt3_model, datetime.now(), temp, pres_pnlt, freq_pnlt, instr_key, instr, aut_obj, gpt3_response_text]
                    writer.writerow(row)
